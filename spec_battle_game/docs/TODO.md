...
### 1. キャラクター図鑑・対戦履歴 (Phase 3-2)
- **概要**: `collection_screen.dart` を新設し、過去に戦った敵のデータや自分のキャラの育成履歴を閲覧できるようにする。
- **作業内容**:
  - `shared_preferences` または新規パッケージを用いて、対戦履歴データを永続化する。
  - ホーム画面から図鑑画面への導線（ボタン追加）を作成する。
  - 図鑑画面のUI構築（リスト表示やグリッド表示、未遭遇キャラのシルエット化など）。
