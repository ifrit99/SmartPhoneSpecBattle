...
- **次に取り組むべきタスク**:
  ...
  - Phase 5 ブラッシュアップ仕様策定 → **ユーザー承認済み**（2026-07-02: `docs/phase5_brushup_spec.md`。分析基盤/エラー監視/空状態UX/ホーム分割/バックアップv2/オンラインランキングの要件定義・機能設計・画面設計・テスト観点。次は §5 の計画ファイル順に実装着手）
  - agmsg エージェント間連携ハーネスを構築（2026-07-06、`feature/agmsg-harness`）
    - チーム specbattle（claude=オーケストレーター、codex=実装担当）、セットアップ/起動スクリプト（`scripts/agmsg/`）
    - 設計=agmsg-designer / レビュー=agmsg-reviewer のサブエージェント分離、画像生成はCodexのimage gen
    - 運用仕様: `docs/agmsg_harness.md`
  - 別タスク候補（未着手）:
    ...
    - ~~`assets/fonts/NotoSansJP-Regular.ttf` の pubspec 登録 or 削除の判断~~ → アプリ内グリフのサブセットを `NotoSansJP` として登録（全量 TTF はクリティカルパスに載せない）
    - バックアップコードへのチェックサム付与（バトルコードと同等の整合性検証）
    - LICENSE ファイルの追加判断
    - 分析基盤・エラー監視の導入（release-monetization-assessment.md 参照）
...
- **現在の位置づけ**: character-art-detail in progress. F6 オンラインランキングは計画済み・未着手（`docs/plans/online-ranking.md`。PR-0 のユーザー作業と未決事項の判断待ち）。
...
| バトル開始時の支援コマンド選択 | ✅ |
| リザルト画面のもう一戦/ガチャ導線 | ✅ |
| デイリーミッション/ショップの並列受取・購入の排他制御 | ✅ |
| audio unlock 失敗時の再試行可能化 | ✅ |
...
## 既知の問題・課題
- ~~**ユニットテスト環境構築エラー（Mac）**~~: 解消済み（2026-06-10 時点で Mac ローカルの `flutter test` 全327件パスを確認。Flutter 3.44.0 への更新で再発しなくなった）。
