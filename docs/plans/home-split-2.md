...
本PR（home-split-2）は段階分割の第2PR。2〜4カードのみ抽出する。完了定義（800行以下）は後続PRを合わせた目標であり、本PR単体では達成しない。
...
---
## Generator ログ
- 第2PRとして `DailyMissionCard` / `RivalRoadCard` / `SeasonPassCard` の3枚を `lib/presentation/widgets/home/` へ抽出。いずれも Stateless。表示データとコールバックはコンストラクタ経由。状態・永続化は `HomeScreen` のまま。
